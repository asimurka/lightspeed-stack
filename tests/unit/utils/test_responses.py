...
class TestParseReferencedDocuments:
    ...
    def test_parse_referenced_documents_file_search_call(
        ...
        mock_result1.attributes = {"link": "https://example.com/doc1"}

        mock_result2 = {
            "filename": "doc2.pdf",
            "attributes": {"url": "https://example.com/doc2"},
        }
    ...
    def test_parse_referenced_documents_message_annotations(
        ...
        assert result[1].doc_url is None
        assert result[1].doc_title == "doc2.pdf"

    def test_parse_referenced_documents_string_parts_skipped(
        self, mocker: MockerFixture
    ) -> None:
        """Test that string parts in message content are skipped."""
        ...
        result = parse_referenced_documents(mock_response)
        # Should only parse from mock_part, not from string part
        assert len(result) == 1
        assert result[0].doc_url == AnyUrl("https://example.com/doc1")
    ...
    def test_parse_referenced_documents_deduplication(
        ...
        mock_result.filename = "doc1.pdf"
        mock_result.attributes = {"link": "https://example.com/doc1"}
